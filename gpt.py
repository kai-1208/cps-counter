...
class CPSMeasurementScreen(QWidget):
    ...
    def start_measurement(self, duration):
        self.click_count = 0
        self.max_cps = 0
        self.elapsed_timer = QElapsedTimer()  # ここで新しいタイマーオブジェクトを作成
        self.duration = duration * 1000  # ミリ秒に変換
        self.cps_history = []
        self.timer = QTimer()
    ...
    def mousePressEvent(self, event: QMouseEvent):
        if not self.elapsed_timer.isValid():
            ...
            QTimer.singleShot(self.duration, self.finish_measurement)  # ここで測定時間を開始
        self.click_count += 1
    ...
    def update_cps(self):
        ...
        if elapsed_time > 0:
            ...
            if len(self.cps_history) >= 10:  # 1秒ごとに記録
                last_10_intervals = self.cps_history[-10:]  # 最新10個（約1秒分）取得
                avg_last_1s_cps = sum(last_10_intervals) / len(last_10_intervals)
                self.max_cps = max(self.max_cps, avg_last_1s_cps)  # 1秒平均を最大 CPS として記録
